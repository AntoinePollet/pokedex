<template>
    <div class="h-[64px] w-full bg-amber-300 dark:bg-dark-1 border-b border-solid border-dark-3 fixed z-50">
        ...
        <div class="sm:hidden flex justify-start items-center h-full pl-3">
            <span @click="toggleMenu" class="material-symbols-outlined cursor-pointer dark:text-white" style="font-size: xxx-large">
                menu
            </span>
        </div>
        ...
        <div v-if="isMenuOpen">
            <div class="sm:hidden h-screen w-screen bg-[#F2F2F2] dark:bg-dark-2 z-50 fixed p-4">
                <div class="flex flex-col items-center gap-y-3">
                    <router-link class="hover:text-amber-800 font-bold dark:text-white dark:hover:text-amber-300" to="/" @click="isMenuOpen = false">Pokedex</router-link>
                    <router-link class="hover:text-amber-800 font-bold dark:text-white dark:hover:text-amber-300" to="/team" @click="isMenuOpen = false">Team</router-link>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        return { teamSize, toggleDark, isDark, toggleMenu, isMenuOpen }
    }
})
</script>
